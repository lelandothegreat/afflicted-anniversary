--[[
	Afflicted 3
	Compatible with TBC Anniversary (BCC) and retail.
]]

-- CopyTable may be missing on some clients (e.g. older Classic)
...
function Afflicted:OnInitialize()
	...
	-- Build spell name -> ID map for BCC/Classic where combat log often returns spellID=0
	self.spellNameToId = {}
	for spellID in pairs(self.db.profile.spells) do
		if type(spellID) == "number" then
			local name = GetSpellInfo(spellID)
			if name then self.spellNameToId[name] = spellID end
		end
	end
...
-- Effective spell ID for timer keys (BCC/Classic often has spellID=0; use spellName then)
function Afflicted:EffectiveSpellId(spellID, spellName)
...
function Afflicted:GetSpell(spellID, spellName)
	-- BCC/Classic: combat log can return spellID=0; resolve by name
	if (not spellID or spellID == 0) and spellName and self.spellNameToId then
		spellID = self.spellNameToId[spellName]
	end
	if( self.spells[spellName] ) then
		return self.spells[spellName]
	elseif( spellID and tonumber(self.spells[spellID]) ) then
		return self.spells[self.spells[spellID]]
	end
	return spellID and self.spells[spellID] or nil
end
...
function Afflicted:COMBAT_LOG_EVENT_UNFILTERED(event)
	...
	if (eventType == "SPELL_AURA_REMOVED" and (not self.db.profile.hostileOnly or sourceEnemy)) then
		...
		self:AbilityEarlyFade(sourceGUID, sourceName, spell, self:EffectiveSpellId(spellID, spellName), spellName)
		end
	...
	elseif (eventType == "SPELL_SUMMON" and (not self.db.profile.hostileOnly or sourceEnemy)) then
		...
		if( spell and spell.type == "totem" ) then
			local effectiveId = self:EffectiveSpellId(spellID, spellName)
			...
			summonedObjects[destGUID] = sourceGUID .. effectiveId
		...
		summonedTotems[id] = sourceGUID .. (self:EffectiveSpellId(spellID, spellName))
...
function Afflicted:ResetCooldowns(sourceGUID, resets, spellData)
	for _, spellID in pairs(resets) do
		...
		local effectiveId = self:EffectiveSpellId(spellID, GetSpellInfo(spellID))
		self[self.db.profile.anchors[spellData.cdAnchor].display]:RemoveTimerByID(sourceGUID .. effectiveId .. "CD")
		end
	end
end

-- Timer started (spellNameFromLog optional; used when spellID is 0 in BCC/Classic)
function Afflicted:AbilityTriggered(sourceGUID, sourceName, spellData, spellID, spellNameFromLog)
	...
	-- Grab spell info (BCC/Classic can have spellID=0; use spellNameFromLog then)
	local spellName, _, spellIcon = GetSpellInfo(spellID)
	spellName = spellName or spellNameFromLog
	local effectiveId = self:EffectiveSpellId(spellID, spellName)
	...
	if( not spellData.disabled and spellData.anchor and spellData.duration > 0 ) then
		self:CreateTimer(sourceGUID, sourceName, spellData.anchor, spellData.repeating, false, spellData.duration, effectiveId, spellData.name or spellName, spellIcon)
	...
	if( not spellData.cdDisabled and spellData.cdAnchor and spellData.cooldown > 0 ) then
		self:CreateTimer(sourceGUID, sourceName, spellData.cdAnchor, false, true, spellData.cooldown, effectiveId, spellData.cooldownName or spellName, spellIcon)
