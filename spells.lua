...
Afflicted.modules.Spells = {version = 18}
...
function Spells:GetData()
	...
	-- Embedded TBC arena-relevant cooldowns (copied from OmniBar_TBC, but kept local
	-- so Afflicted does not depend on any other addon at runtime).
	local OmniBarTbcCooldowns = {

		-- General
		[42292] = { duration = 300, class = "GENERAL" }, -- PvP Trinket
		...
		[6552] = { duration = 10, class = "WARRIOR" }, -- Pummel (Rank 1)
			...
			[72] = { parent = 6552 }, -- Shield Bash (Rank 1)
			[1671] = { parent = 6552 }, -- Shield Bash (Rank 2)
			[1672] = { parent = 6552 }, -- Shield Bash (Rank 3)
			[29704] = { parent = 6552 }, -- Shield Bash (Rank 4)
	...
	-- Merge embedded OmniBar_TBC style cooldowns into Afflicted's spell table.
	for spellId, info in pairs(OmniBarTbcCooldowns) do
		if type(spellId) == "number" and not self.spells[spellId] then
			if info.parent and type(info.parent) == "number" then
				-- Rank that shares cooldown with a parent spellId.
				self.spells[spellId] = info.parent
			else
				local duration = info.duration
				local adjust = info.adjust or 0
				local cd = type(duration) == "number" and (duration + adjust) or nil
				if cd and cd > 0 then
					local class = info.class
					if class and class ~= "GENERAL" then
						self.spells[spellId] = string.format("{cooldown=%g;cdAnchor='cooldowns';class='%s';}", cd, class)
					else
						self.spells[spellId] = string.format("{cooldown=%g;cdAnchor='cooldowns';}", cd)
					end
				end
			end
		end
...
function Spells:GetTotemClass(spellName)
	if( not self.totems ) then
		...
		-- Helper to safely register totems only if the spell exists on this client
		local function addTotem(id, school)
			local name = GetSpellInfo(id)
			if name then
				self.totems[name] = school
			end
		end
